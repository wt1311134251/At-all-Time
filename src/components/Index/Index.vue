<template>
    <div class="zlf_index">
<!--      海报区-->
      <nav>
        <van-swipe :autoplay="3000" indicator-color="white">
          <van-swipe-item><img src="https://msmk2019.oss-cn-shanghai.aliyuncs.com/uploads/image/2019XXAW4dyinn1575882641.jpg" alt=""></van-swipe-item>
          <van-swipe-item><img src="https://msmk2019.oss-cn-shanghai.aliyuncs.com/uploads/image/20197Cxc53hktC1569839552.jpg" alt=""></van-swipe-item>
          <van-swipe-item><img src="https://msmk2019.oss-cn-shanghai.aliyuncs.com/uploads/image/2019XXAW4dyinn1575882641.jpg" alt=""></van-swipe-item>
          <van-swipe-item><img src="https://msmk2019.oss-cn-shanghai.aliyuncs.com/uploads/image/2019XXAW4dyinn1575882641.jpg" alt=""></van-swipe-item>
        </van-swipe>
      </nav>
<!--      这里是分类-->
      <ul class="zlf_class" >
        <router-link tag="li" to="/course">
          <img alt="" src="data:image/png;base64,">
            <p>特色课</p>
        </router-link>
        <router-link tag="li" to="/oto">
          <img src="data:image/png;base64," alt="">
          <p>一对一辅导</p>
        </router-link>
        <router-link tag="li" to="#">
          <img src="data:image/png;base64," alt="">
          <p>学历日历</p>
        </router-link>
      </ul>
<!--      明星讲师列表-->
      <section class="zlf_list">
        <h1>明星讲师</h1>
        <router-link tag="dl" v-for="(item ,key) in 10" :key="key" to="/teacher">
         <dt>
           <img src="uiuiu" alt="这里是左侧的图片">
         </dt>
          <dd>
            <h2>小华华 <span>M11</span></h2>
            <p>教学特色：我们坚持以社会主义，毛泽东思想  </p>
          </dd>
        </router-link>
      </section>
<!--      名师阵容-->
      <section class="zlf_list">
        <h1>名师阵容</h1>
        <router-link tag="dl" v-for="(item ,key) in 10" :key="key" to="/teacher">
          <dt>
            <img src="uiuiu" alt="这里是左侧的图片">
          </dt>
          <dd>
            <h2>小华华 <span>M11</span></h2>
            <p>教学特色：我们坚持以社会主义，毛泽东思想  </p>
          </dd>
        </router-link>
      </section>
    </div>
</template>

<script>
    export default {
        name: "Index",
      data(){
          return {
            images: [
              'https://msmk2019.oss-cn-shanghai.aliyuncs.com/uploads/image/2019XXAW4dyinn1575882641.jpg',
              'https://msmk2019.oss-cn-shanghai.aliyuncs.com/uploads/image/20197Cxc53hktC1569839552.jpg'
            ]
          }
      }
    }
</script>

<style lang="scss" scoped>
      .zlf_index{
        height: 100%;
        background: #eee;
        display: flex;
        flex-direction: column;
        overflow-y: auto;
        //海报样式
        nav{
          img{
            width: 100%;
            height: 4rem;
          }
        }
      }
  //  分类样式
      .zlf_class{
        height: 2.06rem;
        display: flex;
        text-align: center;
        justify-content: space-evenly;
        position: relative;
        top: -0.4rem;
        li{
          width: 2.06rem;
          height: 2.06rem;
          background: #fff;
          border-radius: 0.15rem;
          p{
            font-size: 0.24rem;
            color: #8c8c8c;
          }
          img{
            width: 0.42rem;
            height: 0.4rem;
            margin: 0.4rem 0 0.2rem 0;
          }
        }
      }
  /*    列表样式*/
  .zlf_list{
    padding: 0 0.2rem;
    flex-grow: 1;
    &:nth-of-type(2){
      margin-top: 0.3rem;
    }
    h1{
      font-weight: normal;
      font-size: 0.3rem;
      border-left: 0.06rem solid red;
      padding-left: 0.18rem;
    }
    dl{
      display: flex;
        width: 6.5rem;
        /*height: 1.62rem;*/
         padding: 0.38rem 0.3rem;
        background: #fff;
        border-radius: 0.05rem;
        margin-top: 0.2rem;

      dt{
        img{
          width: 0.8em;
          height: 0.8rem;
          vertical-align: middle;
          border-radius: 50%;
          margin-right: 0.2rem;
        }
      }
      dd{
        h2{
          font-weight: normal;
          font-size: 0.28rem;
          height: 0.48rem;
          span{
            font-weight: normal;
            font-size: 0.18rem;
            color: #e65f04;
          }
        }
        p{
          font-size: 0.22rem;
          text-indent: 1em;
          color: #b7b7b7;
          width: 5rem;
          overflow:hidden; /*超出的部分隐藏起来。*/
          white-space:nowrap;/*不显示的地方用省略号...代替*/
          text-overflow:ellipsis;/* 支持 IE */
        }
      }
    }

  }


</style>
